...
class BroAccessibilityService : AccessibilityService() {
    ...
    fun typeText(text: String) {
        ...
        val t = root.findFocus(AccessibilityNodeInfo.FOCUS_INPUT) ?: findEditable(root) ?: return
        t.performAction(AccessibilityNodeInfo.ACTION_SET_TEXT,
    ...
    fun readScreen(): String {
        ...
        return extractText(root).take(500)
    }
    private fun findEditable(n: AccessibilityNodeInfo): AccessibilityNodeInfo? {
        if (n.isEditable) return n
        for (i in 0 until n.childCount) { val r = findEditable(n.getChild(i) ?: continue); if (r != null) return r }
        return null
    }
    private fun extractText(n: AccessibilityNodeInfo): String {
        val sb = StringBuilder()
        if (!n.text.isNullOrEmpty()) sb.append(n.text).append(" ")
        for (i in 0 until n.childCount) sb.append(extractText(n.getChild(i) ?: continue))
        return sb.toString()
    }
}
